...
function LeftSidebar({isOpen, onToggle, isBlock}: LeftSidebarProps) {
  ...
  const linkClass = `w-full bg-white w-max-[100px] py-2 px-1 text-nowrap flex items-center justify-center ${isBlock ? "" : "md:border-2 flex-col"}  border-gray-300 rounded-md transition duration-300 cursor-pointer hover:bg-gray-200 hover:text-black`

  const links = (
    <>
      <ScrollToTopLink to="/" className={`${linkClass}`}> <FaHome/>Trang chủ</ScrollToTopLink>
      <ScrollToTopLink to="/productpage/" className={`${linkClass}`}> <FaShoppingBasket/>Trang sản phẩm</ScrollToTopLink>
    </>
  )
  ...
  return (
    <div className={`z-40 ${setHidden}`}>
      {/* Sidebar */}
      <div className={`fixed ${isBlock ? "xl:translate-x-[5%] transform translate-none" : " border-2"} bottom-767-css md:top-16 left-0 w-32 bg-white transition-transform duration-300 z-40 border-gray-300 rounded
          ${translate}
        ...
        <nav className={`flex ${isBlock ? "md:flex-row" : "md:flex-col md:mt-16 md:p-4"} flex-row gap-1 md:gap-4 lg:mt-0 justify-evenly ssm:justify-center md:justify-between`}>
          <div className={`flex flex-row flex-1 ${isBlock ? "lg:flex-row" : "lg:flex-col"} gap-1 md:gap-4 justify-evenly ssm:justify-center md:justify-between`}>
            ...
            <ScrollToTopLink to="/categories" className={`${linkClass} p-2 lg:hidden`}><MdMenuBook/>Danh mục</ScrollToTopLink>
          </div>
        </nav>
      </div>
